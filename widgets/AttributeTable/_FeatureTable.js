...
define([
  ...
  return declare([_WidgetBase, Evented], {
        ...
        if (extent && extent.spatialReference && extent.spatialReference.isWebMercator()) {
          normalizeUtils.normalizeCentralMeridian(
            [extent], null, lang.hitch(this, function(normalizedGeo) {
              ...
              this._popupMessage(err.message || err);
            }));
        } else {
    ...
    _getLayerObject: function() {
      return this.layerInfo.getLayerObject()
    ...
    _queryOnClient: function(normalizedExtent, pk, selectionRows) {
      ...
      if (normalizedExtent && esriConfig.defaults.geometryService) {
        ...
        esriConfig.defaults.geometryService.relation(
          ...
          }, json), lang.hitch(this, this._errorGeometryServices));
      } else {
        this.queryExecute(
          selectionRows,
          json.fields, json.features.length, false, json
    ...
    _getFeatureCount: function(normalizedExtent) {
      ...
      this.layer.queryCount(query).then(lang.hitch(this, function(count) {
        if (!this.domNode) {
          return;
        }
        def.resolve(count);
    ...
    _queryFeatureLayer: function(normalizedExtent, pk, selectionRows,
      ...
      qt.execute(
        ...
        lang.hitch(this, this.queryExecute,
          selectionRows, oFields, recordCounts, exceededLimit
        ),
        lang.hitch(this, this._errorQueryTask)
      );
    },
    ...
    _getFeatureIds: function(pk, normalizedExtent) {
      ...
      this.layer.queryIds(query).then(lang.hitch(this, function(objectIds) {
        if (!this.domNode) {
          return;
        }
    ...
    queryExecute: function(selectionRows, oFields, recordCounts, exceededLimit, results) {
      var data = [];
      var store = null;
      var columns = {};
      if (!this.domNode) {
    ...
    createTable: function(columns, store, recordCounts) {
      if (this.grid) {
        ...
        this.grid = new(declare(
          [OnDemandGrid, Selection, ColumnHider, ColumnResizer]
    ...
    _getExportData: function() {
      ...
      if (data && data.length > 0) {
        ...
        if (store instanceof Memory) {
          ...
          this._getExportDataFromServer(_outFields)
    ...
    _getExportDataFromServer: function(outFields) {
      var def = new Deferred();
      var qt = new QueryTask(this.layer.url);
      var query = new Query();
      query.where = this._getLayerFilterExpression();
