...
function (
	...
	all, date, locale, Select, TextBox, NumberTextBox, DrawBox, LoadingShelter, 
	...
	upstreamNavigationSucceeded: function(data) 
	{
		...
		if (data.us_huc12_ids.value.length > 0)
		{
			...
			if (dom.byId("showAllHUC12") != null && dom.byId("showAllHUC12").checked) 
			{
				...
				// for the EPA version the chunk size has to be 90, for the EnviroAtlas, it can be 
				var i, j, temparray;
				
				for (i = 0, j = data.us_huc12_ids.value.length; i < j; i += this.huc12_array_slice_size) 
				{
				    temparray = data.us_huc12_ids.value.slice(i, i+ this.huc12_array_slice_size);
				    
				    // do whatever
					var query12 = new Query();
					query12.where = "HUC_12 in ('" + temparray.join("','") + "')";
					query12.returnGeometry = true;
					query12.outFields = ["HUC_12"];
					
					this.qtHUC12 = new QueryTask(this.huc12_mapserver);
					//force the request to use POST
					this.qtHUC12.requestOptions = {usePost: true};
					var exHUC12 = this.qtHUC12.execute(query12);
					
					deferred_queries.push(exHUC12);					    
				}

				my_len = data.us_huc12_ids.value.length - 1
				console.log("There are " + my_len + " upstream HUC12s. Running " + deferred_queries.length + " HUC12 Upstream GIS queries with " + this.huc12_array_slice_size + " HUC12s in each query.");

			}
			else
			{
				// this function extracts the HUC12s that are within the HUC8 of the selected (clicked) HUC12
				// this gets called when the user chooses to see the navigation results using HUC8 - way quicker!!!
				huc12_ids = this.withinMyHUC8_slice(data.us_huc12_ids.value, 90, data.huc12.value);
				...
				console.log("There are " + huc12_ids.length + " upstream HUC12s until the HUC8s. Running 1 HUC12 ArcGIS query.");

			}

			// decide if we need to show the HUC8 layer. if so, prepare the queries
			
			//jab - TODO - if we use the EnviroAtlas HUC8 query then we need to 
			// add code to   
			if (huc8_ids.length > 0  & (dom.byId("showAllHUC12") == null || ! dom.byId("showAllHUC12").checked))
			{
				...
				//force the request to use POST
				this.qtHUC8.requestOptions = {usePost: true};
				
				exHUC8 = this.qtHUC8.execute(query8);
				
				deferred_queries.push(exHUC8);
				
				console.log("There are " + huc8_ids.length + " upstream HUC8s. Running 1 HUC8 ArcGIS query.");

				
			}
//			console.time("Running ArcGIS Server Queries");
//			promises = all(deferred_queries);

//	        html.empty(this.divResultMessage);
//	        html.place(html.toDom("Getting HUC12 Graphics"), this.divResultMessage);
...
//		this.navHUC8Layer.name = "Navigated HUC8 Subbasin";
		...
		if (Qresults.length > 1)
		{
			...
			else if (dom.byId("showAllHUC12") == null || ! dom.byId("showAllHUC12").checked)
			{
				...
				this.map.setExtent(graphicsUtils.graphicsExtent(featHUC8));
				
				array.forEach(featHUC8, function(feat)
				{
		...
		else
		{
			...
			this.map.setExtent(graphicsUtils.graphicsExtent(featHUC12));
		}
		...
		array.forEach(Qresults, function(featureSet)
		{
				...
				& featureSet.features[0].attributes.hasOwnProperty('HUC_12') == true)
			{
				...
				{
					featureSet.features[i].setSymbol(that.huc12_symbol());
...
//			html.replaceClass(this.aSelectType.iconNode, 'newSelIcon', 'addSelIcon');
		...
		else if (this.currentFeatures.length > 1)
		{
			html.empty(this.divResultMessage);
			html.empty(this.clickAgainMessage);
			var msg_text = "<label style=\"font-size: 18px;\">" + "Select one of the " + this.currentFeatures.length + " highlighted HUCs to continue navigating and to see additional details." + "</label>";
