				...
				if (layer.hasOwnProperty('eaID') && ((selectedLayerArray[i]) == (layer.eaID.toString()))) {
					...
					if (layer.type.toUpperCase() === 'DYNAMIC') {
						...
						lLayer.setInfoTemplates(popupConfig);
                          ...
                          dojo.connect(this._viewerMap, "onClick", function(evt) {
                               var point = evt.mapPoint;
                              var getFeatureInfoRequest = esri.request({
                              //url: layer.url,
                              url: 
"https://www.mrlc.gov/geoserver/mrlc_display/mrlc_nlcd_2011_landcover_2011_edition_2014_10_10/ows",
                              content: {
                                  "SERVICE":"WMS",
                                "REQUEST": "GetFeatureInfo",
                                "LAYERS":["mrlc_nlcd_2011_landcover_2011_edition_2014_10_10"],
                                "SRS": "EPSG:102113",
                                "BBOX": selfLocalLayer.map.extent.xmin + "," + selfLocalLayer.map.extent.ymin + "," + selfLocalLayer.map.extent.xmax + "," + selfLocalLayer.map.extent.ymax,
                                "WIDTH": selfLocalLayer.map.width,
                                "HEIGHT": selfLocalLayer.map.height,
                                "X": evt.layerX,
                                "Y": evt.layerY,
                                "VERSION":"1.3.0",
                                "QUERY_LAYERS": ["mrlc_nlcd_2011_landcover_2011_edition_2014_10_10"]
                              },
                              handleAs: "text"
                            });
                            getFeatureInfoRequest.then(function(response)
                            {
                                if (!response.includes("InvalidPoint")){
                                  selfLocalLayer.map.infoWindow.setTitle("GetFeatureInfo");
                                  selfLocalLayer.map.infoWindow.setContent(response);
                                  selfLocalLayer.map.infoWindow.resize(140, 120); 
                                  selfLocalLayer.map.infoWindow.show(point);
                                }
                              hideLoading();
                            },
                            function(error)
                            {
                              selfLocalLayer.map.infoWindow.setTitle("GetFeatureInfo - ERROR");
                              selfLocalLayer.map.infoWindow.setContent(error.responseText);
                              selfLocalLayer.map.infoWindow.show(point);
                              hideLoading(error);  
                            });
 
                              
                          })//end of WMS layer on click
                    } 
					//All layer types:
					if (bNeedToBeAdded) {
