...
define([
	...
	var _addSelectedLayers = function(layersTobeAdded, selectedLayerNum) {
		...
		for (i in selectedLayerArray) {
				...
				if (layer.hasOwnProperty('eaID') && ((selectedLayerArray[i]) == (layer.eaID.toString()))) {
					...
					if (layer.type.toUpperCase() === 'DYNAMIC') {
                      ...
                      /*lLayer = new WMSLayer(layer.url, 
                          {format: "png",                        
                        id: window.layerIdTiledPrefix + layer.eaID.toString()
                      });*/
                      /*lLayer = new WMSLayer("https://sampleserver1.arcgisonline.com/ArcGIS/services/Specialty/ESRI_StatesCitiesRivers_USA/MapServer/WMSServer", 
                          {format: "png",                      
                           visibleLayers: [2]
                      });
                      alert("after initiate the layer");
          ...
          this._viewerMap.addLayer(wmsLayer);*///works
      ...
      var resourceInfo = {
        extent: new Extent(-20037508.343,20037508.343, -20048966.104, 20048966.104, {
          wkid: 102100
        }),
        layerInfos: [layer1]
      };
      var wmsLayer = new WMSLayer('https://www.mrlc.gov/geoserver/mrlc_display/mrlc_nlcd_2011_landcover_2011_edition_2014_10_10/wms', {
        resourceInfo: resourceInfo,
        visibleLayers: ['0']
      });
          
          this._viewerMap.addLayers([wmsLayer]);*///not work
...
alert("before add layer");
						this._viewerMap.addLayer(lLayer);
						alert("after add layer");
