  ...
  function (
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      _setConfig: function (config) {
        ...
        if(this.config.popupfrom && this.config.popupfrom === 'webmap'){
          this.popupWebmapRadio.checked = true;
        }else{
          this.popupeSearchRadio.checked = true;
        }
        this.isSpatialLayer.setValue(this.config.spatialsearchlayer || false);
        this.allowCSVExport.setValue(this.config.export2Csv || false);
        this.allowGeoExport.setValue(this.config.export2Geo || false);
        this.allowFCExport.setValue(this.config.export2FC || false);
      ...
      getConfig: function () {
        ...
        var config = {
          ...
          export2Geo: this.allowGeoExport.getValue(),
          export2FC: this.allowFCExport.getValue(),
        ...
        if(this.popupWebmapRadio.checked){
          config.popupfrom = 'webmap';
        }else{
          if(config.popupfrom){
            delete config.popupfrom;
          }
        }
        ...
        var fieldsArray = array.map(rowsData, lang.hitch(this, function (item) {
          ...
          if (item.sumfield) {
            retVal.sumfield = item.sumfield;
            retVal.sumlabel = item.sumlabel;
          }
          if (item.dateformat) {
          ...
          if (item.excludestat) {
            retVal.excludestat = true;
          }else{
            delete retVal.excludestat;
          }
          return retVal;
        }));
      ...
      _initRadios:function(){
        ...
        var group2 = "radio_" + jimuUtils.getRandomString();
        this.popupWebmapRadio.name = group2;
        this.popupeSearchRadio.name = group2;
        jimuUtils.combineRadioCheckBoxWithLabel(this.popupWebmapRadio, this.popupWebmapLabel);
        jimuUtils.combineRadioCheckBoxWithLabel(this.popupeSearchRadio, this.popupeSearchLabel);
      },
      ...
      _onFieldEditOk: function () {
        ...
        if (fieldInfo.sumfield) {
          edits.sumfield = true;
          edits.sumlabel = fieldInfo.sumlabel;
        }
        if (fieldInfo.excludestat){
          edits.excludestat = true;
        }else{
          edits.excludestat = false;
        }
      ...
      _createDisplayField: function (fieldInfo, titleField) {
        ...
        if (fieldInfo.sumfield) {
          rowData.sumfield = fieldInfo.sumfield;
          rowData.sumlabel = fieldInfo.sumlabel;
        }
        if (fieldInfo.excludestat) {
          rowData.excludestat = fieldInfo.excludestat;
        }else{
          rowData.excludestat = false;
        }
...
//        console.info(tr.singleExpression);
